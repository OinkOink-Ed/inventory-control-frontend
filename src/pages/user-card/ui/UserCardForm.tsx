...
import { useUserCardApi } from "@pages/user-card/api/useUserCardApi";
import { editCardUserDtoSchemaZOD } from "@pages/user-card/model/validation/shema.ts";
import {
  useUserCardFormApi,
  useUsersFormApiGetDivision,
  useUsersFormApiGetKabinetsByUserIdForEditUser,
  useUsersFormApiGetRole,
} from "@pages/user-card/api/useUserCardFormApi";
...
export function UserCardForm({ id }: UserCardFormProps) {
  ...
  const selectedDivisions = divisionValue ?? [];
        ...
        >
          ...
          <FormField
            ...
            render={({ field }) => {
              ...
              const selectedKabinetIds = kabinetsValue?.map((k) => k.id);

              const lockedDivisionIds =
                kabinetsData
                  ?.filter((kabinet) =>
                    selectedKabinetIds?.includes(kabinet.id),
                  )
