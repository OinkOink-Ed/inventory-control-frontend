...
import { useChoiseOfKabinetsForCreateUser } from "@/shared/stores";
...
import { InputPhone } from "@/shared/kit";
...
} from "@pages/profile/api/useProfileCardFormApi.tsx";
import { editCardUserDtoSchemaZOD } from "@pages/profile/model/validation/shema.ts";
...
export function ProfileCard() {
  ...
  const form = useForm<PutEditUserDto>({
    mode: "onChange",
    //Нужно будет проверить схемы ZOD
  ...
  const divisionsValue = useWatch({
    name: "division",
    control: form.control,
  });

  const selectedDivisions = divisionsValue ?? [];
          ...
          <FormField
            ...
            render={({ field }) => {
              ...
              const selectedKabinetIds =
                kabinetsValue?.map((k) => k.id) ??
                data.kabinets.map((kab) => kab.id);

              const lockedDivisionIds = kabinetsData
                  ...
                  style={{
                    cursor:
                      (data.role?.roleName !== undefined &&
                        ["user", "staff"].includes(data.role.roleName)) ||
                      isFormDisabled
                        ? "not-allowed"
                        : "pointer",
                  }}
